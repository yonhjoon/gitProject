...
public class Test240117 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		//정답입니다. 2번만에 맞추셨습니다
		//Math.random() * 5+1 => 1~5 난수발생 => trget 이라는 변수 저장
		//count변수 하나 만들어서 몇번 입력했는지 횟수를 누적하면서
		// do = while을 이용해서 반복해서 입력받기 (사용자가 입력한 값이 target과 같은면 탈출)
		
		/*
		int trget = (int)(Math.random() * 5+1); // 1~5사이의 난수 발생
		...
		do {
			...
			count++; // 질문횟수 1 증가
		}while(trget != num); // 사용자가 입력한 값과 컴퓨터가 만들어낸 난수가 다르면 계속 반복
...
//			arr[i] = (int)(Math.random() * 45 + 1); // 1~45사이의 랜덤을 배열에 저장
//		
//			//중복검사
//			if (i > 0 ) {
