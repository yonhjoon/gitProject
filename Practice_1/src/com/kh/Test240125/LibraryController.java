...
public class LibraryController {
	...
	public void insertMember(Member mam) {
		this.mem = mem;
	}
	...
	public Book[] searchBook(String keyword) {
		// keyword가 포함된 도서
		//bList 안의 도서들과 keyword를 비교하여 포함하고있는 경우 tmpArr에 차곡차곡담아라
		Book[] tmpArr = new Book[5];
		int count = 0;
		
		for(int i = 0; i < bList.length; i++) {
			if(bList[i].getTitle().contains(keyword)) { //keyword를 비교하여 포함하고있는 경우 를 뜻하는 경우
	...
	public int rentBook(int index) {
		...
		if (bList[index] instanceof AniBook) { //만화책인 경우
			...
			if (cookBook.isCoupon()) { // 쿠폰유무가 유 일경우
